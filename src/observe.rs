...
impl Metrics {
    pub fn calls(&self)   -> u64 { self.total_calls.load(Ordering::Relaxed) }
    pub fn errors(&self)  -> u64 { self.total_errors.load(Ordering::Relaxed) }
    pub fn latency_us(&self) -> u64 { self.total_latency_us.load(Ordering::Relaxed) }

    /// Mean latency in microseconds (0 if no calls).
    pub fn mean_latency_us(&self) -> u64 {
        let calls = self.calls();
        if calls == 0 { 0 } else { self.latency_us() / calls }
    }
}
...
impl ObserveController {
    ...
    // fusa:req REQ-OBS-004
    pub fn add_hook(&self, f: impl Fn(&Command, &Result<Response, RcpError>, Duration) + Send + Sync + 'static) {
...
mod tests {
    ...
    fn call_count_increments() {
        ...
        for _ in 0..5 { o.send(&cmd, None).unwrap(); }
    ...
    fn error_count_increments_on_failure() {
        ...
        let _ = o.send(&Command { zone: Zone::FRONT_LEFT, ..Default::default() }, None);
    ...
    fn hook_is_called_after_send() {
        ...
        let o = ObserveController::new(ok_ctrl());
        o.add_hook(move |_cmd, _res, _lat| { f2.fetch_add(1, Ordering::SeqCst); });
        let cmd = Command { zone: Zone::FRONT_LEFT, ..Default::default() };
