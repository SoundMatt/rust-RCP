...
impl Controller for ZoneGroup {
    ...
    fn send(&self, cmd: &Command, timeout: Option<Duration>) -> Result<Response, RcpError> {
        let results: Vec<Result<Response, RcpError>> =
            std::thread::scope(|s| {
                let handles: Vec<_> = self.members.iter().map(|m| {
                    let cmd = cmd.clone();
...
mod tests {
    ...
    fn send_broadcasts_to_all_members() {
        ...
        let h1: crate::mock::Handler = Box::new(move |cmd| { c1.fetch_add(1, Ordering::SeqCst); Response { command_id: cmd.id, zone: cmd.zone, status: ResponseStatus::OK, payload: None } });
        let h2: crate::mock::Handler = Box::new(move |cmd| { c2.fetch_add(1, Ordering::SeqCst); Response { command_id: cmd.id, zone: cmd.zone, status: ResponseStatus::OK, payload: None } });
        ...
        g.send(&Command { zone: Zone::CENTRAL, ..Default::default() }, None).unwrap();
    ...
    fn first_member_error_is_propagated() {
        let err_ctrl: crate::mock::Handler = Box::new(|_cmd| {
            Response { command_id: 0, zone: Zone::FRONT_LEFT,
                status: ResponseStatus::ERROR, payload: None }
        });
        // Inject via fault route: build a mock that returns Err
        let h: crate::mock::Handler = Box::new(|_| Response {
            command_id: 0, zone: Zone::FRONT_LEFT,
            status: ResponseStatus::OK, payload: None,
        });
        let m1 = MockController::new(Zone::FRONT_LEFT, Some(Box::new(|_cmd| Response {
            command_id: 0, zone: Zone::FRONT_LEFT, status: ResponseStatus::OK, payload: None,
        ...
        let g = ZoneGroup::new(Zone::CENTRAL, vec![m1, m2]);
        let err = g.send(&Command { zone: Zone::CENTRAL, ..Default::default() }, None).unwrap_err();
    ...
    fn empty_group_send_returns_default_ok() {
        ...
        let resp = g.send(&Command { zone: Zone::CENTRAL, ..Default::default() }, None).unwrap();
