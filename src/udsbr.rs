...
pub trait UdsTransport: Send + Sync {
    fn request(&self, sid: u8, data: &[u8], timeout: Option<Duration>) -> Result<Vec<u8>, RcpError>;
}
...
mod tests {
    ...
    fn uds_bridge_send_ok() {
        ...
        let resp = b.send(&Command { zone: Zone::FRONT_LEFT, cmd_type: CommandType::GET, ..Default::default() }, None).unwrap();
