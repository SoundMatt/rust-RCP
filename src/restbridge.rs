...
pub trait HttpClient: Send + Sync {
    fn post(&self, url: &str, body: &[u8], timeout: Option<Duration>) -> Result<(u16, Vec<u8>), RcpError>;
}

// ── JSON helpers ──────────────────────────────────────────────────────────────

fn cmd_to_json(cmd: &Command) -> Vec<u8> {
    let payload_hex = cmd.payload.as_ref()
        .map(|p| p.iter().map(|b| format!("{:02x}", b)).collect::<String>())
        .unwrap_or_default();
    format!(r#"{{"id":{},"zone":{},"cmd_type":{},"priority":{},"payload":"{}"}}"#,
...
impl RestBridge {
    pub fn new(zone: Zone, client: Arc<dyn HttpClient>, base_url: impl Into<String>) -> Self {
        RestBridge { zone, client, base_url: base_url.into() }
    }
...
impl Controller for RestBridge {
    ...
    fn send(&self, cmd: &Command, timeout: Option<Duration>) -> Result<Response, RcpError> {
        ...
        Ok(Response { command_id: cmd.id, zone: self.zone, status, payload: None })
    }
