...
pub fn decode_response(buf: &[u8], cmd: &Command) -> Result<Response, RcpError> {
    ...
    let msg_type = buf[14];
    let ret_code = buf[15];
    if msg_type != SOMEIP_MSG_RESPONSE {
        return Err(RcpError::Other("not a response".into()));
    }
    let status = if ret_code == 0 {
        ResponseStatus::OK
    } else {
...
impl SomeIpBridge {
    pub fn new(zone: Zone, socket: Arc<dyn SomeIpSocket>, service_id: u16) -> Self {
        SomeIpBridge {
            ...
            socket,
            service_id,
            session: std::sync::atomic::AtomicU16::new(1),
        }
    }
}
...
mod tests {
    ...
    impl SomeIpSocket for MockSocket {
        fn send(&self, _: &[u8]) -> Result<(), RcpError> {
            Ok(())
        }
    ...
    fn header_len_constant() {
        assert_eq!(SOMEIP_HEADER_LEN, 16);
    }
